...
class Logger:
    ...
    PREFIXES: ClassVar[dict] = {
        ...
        "effects": (Colors.BRIGHT_RED, "EffectManager"),
    ...
    def effect_created(self, name: str, effect_id: int) -> None:
        """Log effect creation."""
        ename = self._colorize(f"'{name}'", Colors.BRIGHT_RED)
        eid = self._colorize(str(effect_id), Colors.BRIGHT_CYAN)
        self.success("effects", f"Created {ename} at ID {eid}")
